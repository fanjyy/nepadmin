layui.extend({
    ...
    self.appBody = null;
    self.shrinkCls = 'layadmin-sidebar-shrink';
    self.isInit = false;
    ...
    self.getLoginToken = function(){
        if(self.loginToken == null){
            self.loginToken = self.data()[conf.tokenName] || layui.sessionData(conf.tableName,conf.tokenName);
        }
    ...
    $(document).on('click','[lay-popup]',function(e){
        ...
        self.popup(params.indexOf('{') === 0 ? new Function('return '+ $(this).attr('lay-popup'))() : {url:params});
        return false;
    })
    ...
    self.on('fullscreen',function(e){
        ...
        if(ico.hasClass(normalCls)){
            var e = document.body;
            e.webkitRequestFullScreen ? e.webkitRequestFullScreen() : e.mozRequestFullScreen ? e.mozRequestFullScreen() : e.requestFullScreen && e.requestFullscreen();
            ico.removeClass(normalCls).addClass(activeCls);
        }else{
            var e = document;
            e.webkitCancelFullScreen ? e.webkitCancelFullScreen() : e.mozCancelFullScreen ? e.mozCancelFullScreen() : e.cancelFullScreen ? e.cancelFullScreen() : e.exitFullscreen && e.exitFullscreen();
